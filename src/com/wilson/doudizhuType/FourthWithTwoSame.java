 ...
 * @version 1.0
 */
public class FourthWithTwoSame implements PokerOperatorType {
	...
	public FourthWithTwoSame(Integer number,Integer other,Integer another) {
		this.number = number;
		this.other = other;
				...
				for (Map.Entry<Integer, Integer> entry1 : cards.entrySet()){
					...
					if(anotherKey - key != 0 && entry1.getValue() > 1){
						...
						for (Map.Entry<Integer, Integer> entry2 : newCards.entrySet()){
							...
							if(anotherKey2 - key != 0 && entry1.getValue() > 1 && anotherKey2 >= anotherKey){
								int newValue3 = entry2.getValue() - 2;
								if(newValue3 > 0){
									newCards.put(anotherKey2,newValue3);
								}else{
									newCards.remove(anotherKey2);
								}
								PokerState newState = new PokerState(newCards,pokerState,new FourthWithTwoSame(key,anotherKey,anotherKey2),pokerState.getTurns() + 1);
								result.add(newState);
							}
						}
					}
	...
	public List<PokerState> getBiggerAvailableStates(PokerState pokerState, PokerOperatorType current, int turns) {
		List<PokerState> result = new ArrayList<>();
		if (!(current instanceof FourthWithTwoSame)) {
			return result;
		}
		FourthWithTwoSame triple = (FourthWithTwoSame) current;
		Map<Integer, Integer> cards = pokerState.getCards();
					...
					if(anotherKey - key != 0 && entry1.getValue() > 1){
						...
						for (Map.Entry<Integer, Integer> entry2 : newCards.entrySet()){
							...
							if(anotherKey2 - key != 0 && entry1.getValue() > 1 && anotherKey2 >= anotherKey){
								...
								PokerState newState = new PokerState(newCards,pokerState,new FourthWithTwoSame(key,anotherKey,anotherKey2),turns + 1);
								result.add(newState);
							}
						}
					}
